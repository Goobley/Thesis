...
Using the MALI method with full preconditioning \citep{Rybicki1992} allows multiple atomic species with overlapping transitions to be considered, and implementations of angle-averaged and angle-dependent PRD are present following the method outlined in \citet{Uitenbroek2001}.
RH can also treat molecules and molecular lines outside of instantaneous chemical equilibrium, but we do not make use of this here.

SNAPI also uses a MALI method allowing for overlapping lines, with an implementation of the method entirely distinct to the one used in RH.
