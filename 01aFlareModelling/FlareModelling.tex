...
% To solve \eqref{Eq:KinEq} or \eqref{Eq:StatEq}, we require the values of $P_{ij}$ for the atoms of interest present in the atmosphere (and also any atom with strong transitions that overlap those of an atom we are interested in).
...
In spectral lines, also known as bound-bound transitions there are three processes that need to be understood to develop a model for emissivity and opacity.
These are spontaneous emission, where an atom in an excited state will spontaneously emit a photon and drop to a lower energy state, an absorption process, which is the reverse of spontaneous emission, and stimulated emission, where an atom in an excited state is \emph{induced} to emit by the presence of another photon with the same frequency, direction, and polarisation state as the photon this atom will emit.
...
is the angle-averaged intensity at a frequency.

\subsection{Iterative Solutions}

Now we have an expression for the radiative rates in each line that can be computed numerically given the local value of the intensity, however the radiation field is not known \textit{a priori}.
