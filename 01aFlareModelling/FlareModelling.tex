...
However, it is reasonable to suggest that if the atmosphere is sufficiently collisional then a form of local thermodynamic equilibrium (LTE) holds, whereby local parcels of the plasma are effectively in thermodynamic equilibrium such that Kirchhoff's laws of radiation.
...
Equation \eqref{Eq:KinEq} is frequently simplified to the statistical equilibrium equation, the left-hand side is set to 0 i.e.
\begin{equation}
\sum_{l^\prime\neq l} (n_{l^\prime} P_{l^\prime l}) - n_l \sum_{l^\prime\neq l} P_{ll^\prime} = 0.
\label{Eq:StatEq}
\end{equation}
We will discuss in Chaps.~\ref{Chap:TimeDepRt} and \ref{Chap:2DRT} when the full time-dependent solution need be considered over the simpler statistical equilibrium solution.
...
There are two forms of transition we need to consider for atomic RT, spectral lines, and continua.
...
In general these cross-sections have to be tabulated from laboratory analysis, but are only defined for energies higher than the continuum edge energy.
...
$\Phi$ is the Saha-Boltzmann equation defined as
\begin{equation}
...
The radiative rates that are needed to solve the kinetic or statistical equilibrium equations are an expression of the number of upward ($ij$) and downward ($ji$) due to absorption, spontaneous and stimulated emission processes (and the equivalent bound-free processes).
...
\end{equation}
where $\sigma$ describes the frequency-dependent coherent and isotropic continuum scattering cross-section, and
\begin{equation}
...
Clearly, an iteration scheme will therefore be needed to find a stable set of populations yielding a self-consistent radiation field.
...
The full pre-conditioning approach handles arbitrary overlaps of lines and continua for multiple multi-level atoms.
...
They describe a form of radiative transfer that is still based on a transition by transition description of the problem.
\citet{SocasNavarro1997} proved that this approach can be equivalent to the linearised ALI method of \citet{Scharmer1985} implemented in the MULTI code \citep{Carlsson1986,Carlsson1992} provided the same assumptions are made in the design of the local operator.
...
Whilst there are also more rapidly converging methods (which can be viewed as extensions and variants of the ALI approach) such as the Gauss-Seidel and successive-over-relaxation methods of \citet{TrujilloBueno1995}, (with the extension to multilevel systems presented by \citet{Paletou2007}), the forth-and-back implicit Lambda iteration of \citet{AtanackovicVukmanovic1997} (extended to multilevel systems by \citet{Kuzmanovska2017}), and even the multi-grid method of \citet{FabianiBendicho1997}, we opt for this well-tested method that has proven stable and reliable in a wide variety of situations.
This is particularly key when considering modelling of flares, which present much larger variations in parameters and steeper gradients than the quiet Sun models or academic semi-empirical models often considered, but these other methods present possible avenues of improvement for \Lw{}.
...
\end{equation}
where $i$ and $j$ refer to the atomic levels.
...
\end{align}
for $l\neq l^\prime$.
The problem is now represented by a system of linear equations in the level populations.
...
As discussed previously an update to the populations solving for statistical equilibrium can be phrased as $\Gamma \vec{n} = \vec{0}$.
A variant of this method can also be applied to the time-dependent form of the kinetic equilibrium equations.
We will not directly treat the advection term of equation \eqref{Eq:KinEq}, as this requires consideration of hydrodynamic effects and the discretisation schemes used therein (due to the large gradients of these populations that occur in the solar atmosphere).
An overview of the methods used to numerically solve the conservation laws of hydrodynamics is provided in Sec.~\ref{Sec:IntroConsLaws}.
...
In lower density regions with strong, typically resonance, lines\footnote{A transition whose lower state is the ground state of the atom is known as a resonance transition.} where radiative effects dominate over collisional effects, there is said to be a natural population of a certain level; a population where the emission frequency is correlated to the absorption frequency, and has not been redistributed by collisions \citep{Hubeny2014}.
...
From this definition of $\rho$, under the assumptions of a line with an infinitely sharp lower level and broadened upper level, and the validity of PRD being in the atomic frame being approximated by PRD in the observer's frame \citep{Uitenbroek2001}, following \citet{Hubeny2014} we have
\begin{align}
\begin{split}
...
Its value is then interpolated to find the value of $\rho$ at the Doppler shifted value in the observer's frame.
The additional computational cost of this hybrid method over the angle-averaged method is very low, when compared to the complete angle-dependent method, and in most cases the results are comparable to the complete treatment \citet{Leenaarts2012a, Kerr2019}.
...
\begin{equation}
    ...
    \hspace{2.5em}- \left.\frac{\partial F_{s,i}(\vec{n}_s, n_e)}{\partial n_e}\right\rvert_{(\widetilde{\vec{n}_s}, n_e^\dagger)} \delta n_e,
\end{equation}
...
\end{equation}
where $\mathion_s(j)$ is the ionisation state of the $j$-th level of species $s$, and $n_{e, \mathrm{bg}}$ represents the electron density due to species not considered in detail here.
...
The exact form of these derivatives used in the Jacobian can be computed analytically for all rates (although we resort to a finite-difference method for the collisional rates), and are presented in \citet{Osborne2021}.
...
The iterative process proceeds by following each MALI population update with a secondary Newton-Raphson iteration evaluated using the updated $\Gamma$.
...
The most important quantities to use when estimating convergence are the populations $n$, and $J$ due to its appearance in the source function.
...
As we do not generally have knowledge of the latter, the former is used to determine when to cease iterations.
...
Instead, it is common to use the $L^\infty$ norm of this relative change of these parameters.
For a population $n_i$, following \citet{Auer1994a} we denote this $R_c(itr, i)$ for level $i$ and iteration $itr$.
...
When applied to $J$, the relative change is computed across frequency and atmospheric depth.
...
Nevertheless, we consider that the agreement between the three implementations is very good on this difficult test.
