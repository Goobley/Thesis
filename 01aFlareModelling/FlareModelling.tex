...
As can be seen from the referencing of the previous section, none of these methods are novel on their own -- they represent the most robust methods encountered in our survey of NLTE -- and it is in the combination of these methods and in the implementation strategies employed that \Lw{} differs from current \Sota{} RT codes.
In the following we shall describe the most important design decisions made in \Lw{} and explain how they can enable new forms of RT simulations whilst also increasing productivity.
...
Thus a user with a Python environment (version $\ge3.8$) using an x86-64 CPU supporting AVX vector extensions (essentially any Intel or AMD chip from the past decade) on any of macOS, Windows, or Linux, can install the package in one command using the python package manager \texttt{pip}.
...
A different approach to the problem of needing the specify data and their \emph{associated} methods and approximations is to define a contract describing the information needed by the numerical core and allow the specific model to fulfil this contract through any means.
It is this approach we take in \Lw{}.
This means that models need to be ``smart'' and have the ability to execute arbitrary code.
...
An \texttt{AtomicModel} in \Lw{} is the definition of an object containing an element or isotope identifier, and a list of each of levels, lines, continua, and collisional rates.
Each of these is terms is itself a Python object which must conform to a particular contract describing the form of the information it must be capable of supplying when a particular method is called.
Many of these objects also contain components with their own contracts.
These contracts are defined through the use of Python classes which can be extended with new functionality through inheritance.
A user can therefore implement a new method for computing, say, the absorption profile of a spectral line and employ this by defining a new model atom with no changes needed in the \Lw{} package.

\subsection{Lightspinner}
