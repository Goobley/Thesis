...
Clearly this is not physical and there is a limit maximum limit, known as the free-streaming limit at which all electrons in the plasma are flowing at their thermal speed with this heat gradient, representing a finite limit on this term.
...
\citet{Campbell1984} provides an expression for the transport coefficients used to determine the conductive flux through an ionised plasma and smoothly handles both the Spitzer, locally limited, and non-locally limited regimes.
This approach can be applied in numerical simulations but more frequently (such as in RADYN) the method of \citet{FISHER1985} is applied which smoothly limits the local flux to remain less than the free-streaming limit, hence helping to stabilise this equation.
...
Several alternative methods have been developed, such as reforming the parabolic equation as a hyperbolic wave equation \citep{Rempel2016}, implicit-explicit methods (which may also be used for integrating stiff source terms such as the atomic level population transition rates) which perform operator splitting and use both implicit and explicit methods as needed\citep[e.g.][]{Ascher1995}, or accepting the cost of an explicit discretisation in exchange for simplicity and accuracy \citep{Bradshaw2003, Bradshaw2013}.
...
Simulations using the zero-dimensional enthalpy based EBTEL code currently suggest that this turbulent suppression alone is insufficient to maintain the high coronal temperatures and slow cooling times seen in observations, but likely represents a portion of this effect \citep{Bian2018}.
As part of further investigations, work is currently under way to integrate these effects in the \Radyn{} and HYDRAD codes.
