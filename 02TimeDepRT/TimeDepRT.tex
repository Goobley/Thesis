...
for timeToPlot in plotTimes:
    fig = plot_cfn(timeToPlot, liData, leData, atmost)
...
for timeToPlot in plotTimes:
    fig = plot_cfn(timeToPlot, liDataF10, leDataF10, atmostF10)
...
Looking first at the F9 simulations, shown in Fig.~\ref{Fig:F9LossComparison}, there is a significant difference between the calcium losses in the two treatments during heating (0-\SI{10}{\second}).
This is not surprising given the difference in the outgoing profiles shown in Fig.~\ref{Fig:F9Profiles}.
This difference remain important above an altitude of \SI{1}{\mega\metre} throughout the entire simulation.
