...
#     vmaxAmp = max(vmaxAmp, 0.1)
    ...
    spec = gridspec.GridSpec(nrows=2, ncols=9, width_ratios=[15, 1, 15, 1, 15, 1, 2, 15, 1])
...
This enhancement will both reduce local absorption effects on the line and stimulate enhancements.
...
This enhancement does not appear to be sufficient to directly create the ``horns'' in the \Ha{} profiles, as evidenced by the green line profile, but will interact with the light coming from the flaring boundary condition.

\TODO{Effect on \Caii{} populations is much larger. Can't easily disentangle effects of populations from flare, but can easily see that the flaring effects penentrate deep into the slab.}
