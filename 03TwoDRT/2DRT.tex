...
fig = plt.figure(constrained_layout=True, figsize=texfigure.figsize(pytex, scale=1, height_ratio=0.8))
ctxF10 = plot_rel_change_pops(fig, dataPops, dataRadF10, atmost, 10, hIdxU=2, hIdxL=1, caIdxU=4, caIdxL=2)
...
def plot_radiation_anisotropy(ctx, wlIdx):
    # ctx.depthData.fill = True
    # with open(os.devnull, 'w') as devnull:
    #     with redirect_stdout(devnull):
    #         ctx.formal_sol_gamma_matrices()

    atmos = ctx.atmos.pyAtmos
    sd = deepcopy(ctx.state_dict())
    atmosManyAngle = sd['kwargs']['atmos']
    atmosManyAngle.quadrature(10)
    atmos = atmosManyAngle
    ctxManyAngle = ctx.construct_from_state_dict_with(sd)
    ctxManyAngle.depthData.fill = True
    with open(os.devnull, 'w') as devnull:
        with redirect_stdout(devnull):
            for i in range(100):
                dJ = ctxManyAngle.formal_sol_gamma_matrices()
                if dJ < 1e-3:
                    break
    Idir = ctxManyAngle.depthData.I

    zIdxs = [0]
    for z in range(atmos.height.shape[0]):
        if abs(atmos.height[z] - atmos.height[zIdxs[-1]]) > 1e5:
            zIdxs.append(z)
    ...
    for upDown in [0, 1]:
        ...
        for i in range(Nrays):
            us.append(np.sqrt(1.0 - atmos.muz[i]**2) * Idir[wlIdx, i, upDown, zIdxs])
            vs.append(sign * atmos.muz[i] * Idir[wlIdx, i, upDown, zIdxs])
    # NOTE(cmo): Why doesn't this produce any down directed rays?
    us = np.concatenate(us)
    vs = np.concatenate(vs)

    zs = (atmos.height[zIdxs] / 1e6).tolist() * Nrays * 2
    xs = np.zeros(len(zs))

    fig = plt.figure(figsize=texfigure.figsize(pytex, scale=1, height_ratio=0.7))
    ax = fig.gca()
    ax.quiver(xs, zs, us, vs, scale=1e-6)
    ax.set_ylim(1.2, 1.8)
    return fig

fig = plot_radiation_anisotropy(ctxF10, 927)
lFig = ch2DRT.save_figure('CaF10Anisotropy', fext='.pgf')
\end{pycode}
\py[2DRT]|ch2DRT.get_figure('CaF10Anisotropy')|
%spell-checker: enable
