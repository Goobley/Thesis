...
The boundary condition radiation for the ray at $\mu_z=0.99999$ is computed automatically due to the design of the class that describes it, and the coupling allowed by the framework, which allows easy manipulation and coupling of multiple radiative transfer contexts within the same program.
...
statEq2d_81= zarr.convenience.open(ch2DRT.data_file('F9_flat_450_81_StatEqUpdated_t10_1stColCopy_besser_MsLw2d_thesis.zip'))
...
statEq2d05 = zarr.convenience.open(ch2DRT.data_file('F9_flat_450_41_StatEqUpdated_t05_1stColCopy_besser_MsLw2d_thesis.zip'))
statEq2d05_81= zarr.convenience.open(ch2DRT.data_file('F9_flat_450_81_StatEqUpdated_t05_1stColCopy_besser_MsLw2d_thesis.zip'))
...
statEq2d15_81= zarr.convenience.open(ch2DRT.data_file('F9_flat_450_81_StatEqUpdated_t15_1stColCopy_besser_MsLw2d_thesis.zip'))
...
The maximum difference is found to be an enhancement of around 12\% in both line cores at $\sim$\SI{0.5}{\mega\metre} from the flare for the lower resolution simulation over the higher resolution one.
...
For this reason the 81 point simulation takes $\sim$2.4x longer than the 41 point simulation, and we opt to use the 41 point spatial grid in all models presented in this chapter.
...
These treatments were only compared at one time-step; the process of converging a statistical equilibrium solution with the fixed electron density provided from the time-dependent model proved to

\TODO{See if proper 1stColCopy fixes.}
