...
def plot_rel_change_2d(fig, d, wl, wlOffset, position, angularIdx, enhancementLine=0.4):
    ...
    IChangeArr = (Idata - Idata[:, :, -2][:, :, None]) / Idata[:, :, -2][:, :, None]
...
fig = plt.figure(figsize=texfigure.figsize(pytex, scale=1, height_ratio=0.7))
angularIdx = 1
plot_rel_change_2d(fig, data, 743, 15, 2, angularIdx)
fig.tight_layout()
...
fig = plt.figure(figsize=texfigure.figsize(pytex, scale=1, height_ratio=0.7))
angularIdx = 1
plot_rel_change_2d(fig, data, 927, 7, 2, angularIdx)
fig.tight_layout()
...
def plot_rel_change_pops(fig, data, time, hIdxU, hIdxL, caIdxU, caIdxL):
...
#     vmaxAmp = max(vmaxAmp, 0.1)
    ...
    spec = gridspec.GridSpec(nrows=2, ncols=5, width_ratios=[15, 1, 2, 15, 1])
...
dataPops = zarr.convenience.open(ch2DRT.data_file('F9_flat_450_40_nr_1stColCopy_besser_pops_thesis.zip'))
fig = plt.figure(constrained_layout=True, figsize=texfigure.figsize(pytex, scale=1, height_ratio=0.8))
plot_rel_change_pops(fig, dataPops, 10, hIdxU=2, hIdxL=1, caIdxU=4, caIdxL=2)

lFig = ch2DRT.save_figure('UpperLevelsT10', fig, fext='.pgf', dpi=500)
lFig.caption = r'Relative Changes in the atomic populations from the initial solution at t=\SI{10}{\second} for the upper and lower levels of \Ha{} in the upper plots, and \CaLine{} in the lower plots. The indices used for the levels refer to the level in the model atom used in \Lw{} with zero-indexing.'
