...
This left-hand panel also shows the emergent intensity in this line at $x=\SI{500}{\kilo\metre}$ from the flare at different times throughout the simulation for the viewing angle to the local surface normal in the $x-z$ plane of \SI{2.24}{\degree}.
...
This is likely due to rays passing through positive $x$ region encountering more scattered light from the flare and directly intersecting the flaring atmosphere.
...
A slight asymmetry of the red wing (due to brightening around $\Delta\lambda = \SI{0.05}{\nano\metre}$) starts to appear here in region $A$.
...
The situation is reversed for \CaLine{}; the line core protrudes similarly to \Ha{}'s secondary peaks
a slight recess of approximately \SI{50}{\kilo\metre} at wavelengths just beyond the secondary peaks ($|\Delta\lambda| > \SI{0.02}{\nano\metre}$) before returning smoothly and rapidly towards the continuum values.
...
Nevertheless, there are clear effects due to the two-dimensional simulation beyond \SI{0.25}{\mega\metre} from the flaring boundary, and these lie well within the regions of important variation outlined in the our imaging and spectroscopic analyses of the data.
The further investigate the flare's effect on the slab and why the continuum emission is not affected we will now see how the atomic populations within the slab vary.
...
#     vmaxAmp = max(vmaxAmp, 0.1)
    ...
    with open(os.devnull, 'w') as devnull:
        with redirect_stdout(devnull):
            ...
            for name in ['H', 'Ca']:
                pops = data[name]
                eqPops.atomicPops[name].pops[:] = pops[...][tIdx].reshape(-1, Nz, Nx)[:, :, 1]
...
This approach was taken due to the difficulty difficulty formulating the contribution function in the two-dimensional slab; the wavelength varying contributions of even a single output spectrum from a two-dimensional atmosphere cannot be represented directly on a two-dimensional plots, and this problem of dimensionality becomes greater still when investigating the contribution to each outgoing pixel.
...
The line profiles overplotted on the contribution function are the initial statistical equilibrium solution (dashed black), current emission from the flaring boundary (pink), and those observed from 200 and \SI{400}{\kilo\metre} on either side of the flaring kernel (orange for the positive $x$ region and green for the negative $x$ region).
Whilst it is possible to compute the response function to perturbations in atmospheric parameters, this is not particularly insightful due to the assumption that the temperature and velocity fields remain fixed, and the coupling of the electron density to charge conservation again.
...
The depth of this enhancement decreases with distance from the flaring boundary, and corresponds with the disappearance of the secondary peaks observed over region $B$.
...
The populations are substantially reduced from their initial statistical equilibrium values over this range, with a maximum relative change of $\sim-20$.
...
def plot_ne_radiation_anisotropy_10s(dataPops, ctxBoundary, wlIdxs, lineNames, matchAspectRatios=True):
    ...
    mappable = ax.quiver(xs, zs, us, vs, cs, scale=6e-7, cmap='Reds', angles=angles)
    ax.set_ylim(0.9, 1.7)
...
In Fig.~\ref{Fig:F10AnisotropyNe} the anisotropy of the radiation field leaving the flaring boundary and entering the slab over the range $z=0.9-\SI{1.7}{\mega\metre}$ for the timestep currently considered, along with the electron density in the slab is shown.
...
Their sampling of the unit sphere is far more uniform than it appears here due to this significant stretching effect along $z$ and each vector's projected norm in the $x-z$ plane has been normalised.
...
In the right-hand panel there an approximately triangular region of enhancement in the electron density (which is initially approximately uniform in $x$ from the values at the $x=\SI{2}{\mega\metre}$ boundary), starting at the flaring boundary around $z=\SI{1.1}{\mega\metre}$, and increasing in distance from the flare with altitude up to $x=\SI{0.75}{\mega\metre}$ at $z=\SI{1.7}{\mega\metre}$.
...
This region also appears to overlap well with the incoming Ly$\alpha$ radiation, which will penetrate further at higher altitudes due to the higher temperature and the combination of radiation arriving close to horizontally and from below; this (and other EUV radiation forming at similar temperatures) will likely be the cause of significant photoionisation.
The downward sloped population reduction seen in the \Caii{} population plots corresponds to the line core formation region ($\sim\SI{1.1}{\mega\metre}$) which also aligns with the highest altitude at which \CaLine{} is relatively isotropic.
Above this, the \CaLine{} radiation field is primarily upgoing and injects energy into the slab in a non-uniform fashion.

\Ha{} appears relatively isotropic up to $z=\SI{1.5}{\mega\metre}$, becoming primarily upgoing above this.
...
It is also interesting to note how significantly the strength of the \Caii{} K radiation drops above $z=\SI{1.15}{\mega\metre}$ but remains relatively isotropic until $z\approx\SI{1.3}{\mega\metre}$ and does not appear directly connected to the ionisation region depicted in the right-hand panel.

There is a significant enhancement of electron density within the triangular region shown in the right-hand panel of Fig.~\ref{Fig:F10AnisotropyNe}, which appears to due primarily to photoionisation (as the temperature within the slab cannot change).
...
Region $A$ appears similar in both cases, but there are significant differences in the line width and core depth outside of these regions.
...
We note that the line-core narrowing around $x=\SI{0.5}{\mega\metre}$ in the statistical equilibrium case here is not present in Fig.~\ref{Fig:2dCompareStatEqTimeDep} and must be due to the use of the electron density from the time-dependent model.
...
The maximum fractional error in each panel is shown in the upper-left corner of each panel.
The location of the peak differences between the treatments in the positive and negative $x$ regions appear to follow a pattern similar to those in considered in Sec.~\ref{Sec:2dTimeDepSE}, with the largest differences in positive $x$ appearing in region $B$ while those in negative $x$ appear in $a$.
The maximum difference is found to be an enhancement of around 14\% in both line cores at $x\sim\SI{0.6}{\mega\metre}$ for the ratio of lower resolution simulation to the higher resolution.
...
This scaling is more significant with the addition of points in $z$, which are already very dense in some regions.
...
This data was prepared following the procedure described in Sec.~\ref{Sec:CRISP}.
...
This plot is similar in concept to the imaging plots (Figs.~\ref{Fig:2DHaF9}--\ref{Fig:2DCaF10}) produced from our models, but the ``ribbon'' was held at the $x=0$ boundary.
...
The comparison here is against the line core, so an offset of $0$\,{}px implies that the enhancement in the line core and this wavelength is co-spatial.
...
In both cases, the size of a pixel is similar (\SI{42}{\kilo\metre} for CRISP and \SI{50}{\kilo\metre} in the simulation); whilst the pixels in the data are nominally smaller this effect does not include the effects of viewing the inclined solar surface so we compare these extents in pixels directly.
...
At $\sim\SI{30}{\second}$ the extent of the blue wing enhancement decreases significantly, whilst the red wing remains approximately constant, slightly approaching the line core.
...
The consistency of the red enhancement is likely due to the very strong red asymmetry present in region $A$ of the simulation at later times, whilst the core remains notably enhanced further into the slab.
...
For the F10 simulation, the difference in extent between the line core and the wings remains small during heating, but after the heating ends increases relatively rapidly to $\sim$13\,{}px.
