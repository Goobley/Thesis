...
\begin{tikzpicture}[line width=1pt, >=latex]
    ...
    \draw[->, out=50, in=150] (xSet.90) to node[below] {$f(\vec{x})$} (ySet.90);
    \draw[->, dotted, out=210, in=310] (ySet.270) to node[above] {$f^{-1}(\vec{y})$} (xSet.270);
...
\begin{tikzpicture}[line width=1pt, >=latex]
    ...
    % \draw[->] (xSet.90) to [out=50, in=150] (ySet.90) node[midway] {aa $f(\vec{x})$};
    % \draw[->, dotted] (ySet.270) to [out=210, in=310] (xSet.270) node[midway] {a $f^{-1}(\vec{x})$};
    \draw[->, out=50, in=100] (xSet.90) to node[below] {$g^{-1}(\vec{x})$} (ySet.90);
    \draw[->, dotted, out=260, in=310] (ySet.270) to node[above] {$g(\vec{y}, z)$} (xSet.270);
...
\TODO{Explain how frameworks make this kind of training easy/liken to \Lw{}}
...
\TODO{Add Leaky ReLU explanation}
