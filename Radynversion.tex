...
The total beam deposition varies between \SIrange[range-phrase={~and~}]{3e10}{1e12}{\erg\per\square\cm}, the low-energy cutoff is one of \{10, 15, 20, 25\}~\si{\kilo\electronvolt}, and the spectral index of the electron energy distribution is one of \{3, 4, 5, 6, 7, 8\}.
...
These gradients are used in conjunction with the Adam optimiser \citep{2014Kingma} with $\beta_1=\beta_2=0.8$ and $\epsilon=1\times10^{-6}$, $\beta_i$ control the decay rate for momentum of the first- and second-moment estimates of the gradients and $\epsilon$ simply prevents division by zero.
The learning rate was initialised to $1.5\times0^{-3}$ and decays by a factor of $0.004^{1/1333}$ every 12 epochs.
